...
def log_vite_output(pipe):
    while True:
        ...
        if line:
            if any(word in line.lower() for word in ["error", "failed", "exception", "vite error"]):
...
def run_vite_with_logging(frontend_dir: str):
    ...
    try:
        ...
        logger.error("❌ npm n'est pas trouvé. Vérifiez que Node.js est installé et dans le PATH.")
        return
    ...
    if process:
        ...
        if interrupted:
            ...
            logger.error(f"❌ Vite s'est terminé avec code d'erreur {returncode} (crash inattendu).")
